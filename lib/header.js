...
function encode (obj) {
  ...
  for (var f = 0; fields[f] !== null; f ++) {
    ...
    switch (field) {
      ...
      case "size":
        // a size below zero is not a size at all, so never write one out:
        // it would tell every reader that the entry can never end.  See the
        // matching refusal in decode(), and the one in the extended header
        // writer, which keeps the same value out of a generated PAX record.
        ret = writeNumeric(block, off, end, obj.size < 0 ? 0 : obj.size)
        break
    ...
    switch (field) {
      ...
      default:
        if (numeric[field]) {
          ...
          // The size field is the one number that decides where the next
          // 512-byte header block is looked for, and the base-256 encoding
          // can express a negative one.  A negative size is never valid, and
          // honoring it makes the entry unable to end -- it would count its
          // remaining bytes down from below zero forever.  Take it as no size
          // at all, which for this header is the same as zero.
          if (field === "size" && this[field] < 0) this[field] = 0
        } else {
