...
// \0 does not terminate the record.  Only the length does that.
// It does terminate the value, though: everything from the first \0 on is
// discarded, so that a NUL byte can never be smuggled into a path.
    ...
    switch (this._state) {
      ...
      case VAL:
        ...
        if (this._fieldPos === this._size - 1) {
          ...
          // A \0 terminates the value.  Everything after it is dropped, so
          // that a path or linkpath smuggled in behind a NUL byte can never
          // reach fs.lstat()/fs.open()/fs.symlink(), which reject arguments
          // containing a NUL with an uncaught ERR_INVALID_ARG_VALUE.
          // Unlike the record framing in newer tars, a value here may span
          // a \n, so match the tail with [\s\S] rather than . -- otherwise a
          // second NUL on a later line would survive.
          val = val.replace(/\0[\s\S]*/, "")
          if (numeric[this._key]) {
            val = parseFloat(val)
          }
          // A "size" record is only kept when it is a number that is zero or
          // greater.  A negative size can never be satisfied: the entry it
          // describes counts its remaining bytes down to exactly 0, so
          // starting below zero means the entry never ends, it swallows every
          // block that follows, and the parser never looks for another header.
          // Drop the record entirely -- as if the PAX block had never carried
          // it -- rather than storing it.  A value that does not parse as a
          // number fails the same test and is dropped too.
          if (this._key !== "size" || val >= 0) {
