...
function Extract (opts) {
  ...
  me.on("entry", function (entry) {
    ...
    if (entry.props && entry.props.linkpath) {
      ...
      // hardlink targets are resolved against the root of the tarball, so a
      // '..' component always reaches outside of it.  symlink targets are
      // resolved against the entry's own directory, where '..' is legitimate
      // (and kept inside the extraction dir by the SymbolicLink block below).
    ...
    if (entry.type === "SymbolicLink") {
      ...
      // against opts.path treats a sibling dir that merely starts with the same
      // name (eg /tmp/target-evil vs /tmp/target) as being inside the target.
      var normalizedTarget = target.replace(/\\/g, '/')
      if (normalizedTarget.indexOf(normalizedExtract + '/') !== 0 &&
        normalizedTarget !== normalizedExtract) {
