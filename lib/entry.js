...
Entry.prototype._setProps = function () {
  ...
  // A size below zero cannot be honored, wherever it came from -- the header
  // block, an extended header, or a global extended header.  write() ends the
  // entry when _remaining reaches exactly 0, so counting down from a negative
  // number never gets there: the entry keeps accepting blocks, the parser
  // never looks for the next header, and every remaining entry in the archive
  // is swallowed.  Treat it as no size at all.
