...
Parse.prototype._startEntry = function (c) {
  ...
  // A pending extended header describes the *next file system entry*, not
  // whatever other block happens to come next.  Only apply it when this
  // block's own type flag is a normal file system type.  Blocks that are
  // not (sparse files, tape volume headers, Solaris ACLs, inode metadata,
  // continuation files, and any unrecognized type flag) must be measured by
  // their own header: honoring a pending PAX "size" for them moves where the
  // next 512-byte header is expected, desynchronizing this parser from other
  // tar implementations, and allowing entries to be smuggled past them.
  var normalFs = tar.normalFsTypes[header.type] === true

  var global, extended
  if (meta) {
    global = extended = null
  } else {
    var pendingExtended = this._extended
